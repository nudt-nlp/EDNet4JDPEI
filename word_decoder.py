...
def read_data(src_file, trg_file, datatype):
    reader = open(src_file, 'r', encoding='utf-8')
    src_lines = reader.readlines()
    reader.close()

    reader = open(trg_file, 'r', encoding='utf-8')
...
def predict(samples, model, model_id):
    ...
    for batch_idx in tqdm(range(0, batch_count)):
        ...
        with torch.no_grad():
            if model_id == 1:
                outputs = model(src_words_seq, src_chars_seq, src_words_mask, trg_words_seq, trg_vocab_mask, False)
...
def train_model(model_id, train_samples, dev_samples, best_model_file):
    ...
    for epoch_idx in range(0, num_epoch):
        ...
        for batch_idx in tqdm(range(0, batch_count)):
            ...
            target1 = torch.as_tensor(target, dtype=torch.long)
...
if __name__ == "__main__":
    ...
    if job_mode == 'test':
        ...
        ref_lines = open(trg_test_file, encoding='utf-8').readlines()
