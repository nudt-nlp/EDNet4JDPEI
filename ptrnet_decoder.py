...
def read_data(src_file, trg_file, datatype):
    reader = open(src_file, encoding='utf-8')
    src_lines = reader.readlines()
    reader.close()

    reader = open(trg_file, encoding='utf-8')
...
class Seq2SeqModel(nn.Module):
    ...
    def forward(self, src_words_seq, src_mask, src_char_seq, pos_seq, trg_words_seq, trg_rel_cnt, arg1_mask, arg2_mask, is_training=False):
        if is_training:
...
def train_model(model_id, train_samples, dev_samples, best_model_file):
    ...
    for epoch_idx in range(0, num_epoch):
        ...
        for batch_idx in tqdm(range(0, batch_count)):
            ...
            loss = rel_criterion(outputs[0], torch.as_tensor(rel, dtype=torch.long)) + \
                   wf * (pointer_criterion(outputs[1], torch.as_tensor(arg1s, dtype=torch.long)) + pointer_criterion(outputs[2], torch.as_tensor(arg1e, dtype=torch.long))) + \
                   wf * (pointer_criterion(outputs[3], torch.as_tensor(arg2s, dtype=torch.long)) + pointer_criterion(outputs[4], torch.as_tensor(arg2e, dtype=torch.long)))
...
if __name__ == "__main__":
    ...
    max_src_len = 900    # GPU: A100(80g) others: e.g.350
    max_trg_len = 2000   # e.g. 100
    ...
    rel_file = os.path.join(src_data_folder, 'interactions.txt')
    ...
    if job_mode == 'test':
        ...
        reader = open(os.path.join(src_data_folder, 'test.tup'), encoding='utf-8')
